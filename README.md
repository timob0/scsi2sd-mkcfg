...
- The image files on the SD card must be stored on contigous blocks. This usually is ensured when adding images to a freshly formatted card. Once you start deleting and replacing images, chances a very high that they get fragmented (=stored in multiple pieces on different locations on the card) which makes them unusable on the target system and very likely will lead to corruption of other images on the card. To work around this, when you have to replace images, just rename the old one, add a different suffix (.old) and add the updated image to the card. From time to time, backup the SD card to a computer, format it and add back the files, leaving out the to be deleted ones. This way you will make sure that files always occupy contigous space.
...
- Align with bluescsi naming convention (disk type and SCSI id are part of the filename)
